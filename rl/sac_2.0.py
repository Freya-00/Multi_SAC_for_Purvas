...
class SACnetworks(object):
    def __init__(self,
        ...
        if self.share_action:
            self.policy_net = PolicyNetwork(state_dim - self.action_num, action_dim, hidden_dim).to(device)
        else:
    ...
    def networks_update(self,
        ...
        if self.share_action == True:
            state_policy = state[:,:-self.action_num]
        else:
            state_policy = state
        ...
        new_action, log_prob, z, mean, log_std = self.policy_net.evaluate(state_policy)
